<template>
  ...
  <nav class="sticky top-0 bg-white/90 backdrop-blur-md shadow-sm z-50">
    <div class="container mx-auto px-4 lg:px-8">
      <div class="flex justify-between items-center h-16 lg:h-20">
        <div class="flex items-center">
          <RouterLink
            to="/"
            class="text-2xl lg:text-3xl font-bold gradient-text"
            >MUXEE</RouterLink
          >
        </div>
        ...
        <div class="hidden lg:flex items-center space-x-8">
          <RouterLink
            ...
            :to="item.path"
            :class="{
              'text-primary font-medium': item.path == currentRoute,
    ...
    <!-- Mobile Menu -->
    <div v-show="isShowMobileMenu" class="bg-white border-t">
      <div class="px-4 py-2 space-y-1">
        <RouterLink
          ...
          :class="{
            'text-primary font-medium': item.path == currentRoute,
            'text-gray-700': item.path != currentRoute,
          }"
          >{{ item.label }}</RouterLink
        >
      </div>
...
function switchLanguage() {
  let newLocale = locale.value === 'zh' ? 'en' : 'zh';
  try {
    // 1. 更新 I18n 实例的当前语言
...
onMounted(() => {
  ...
  console.log("--onMounted--", currentRoute)
})

// 导航数据 为了切换语言后实时更新，使用计算属性
// const menu = ref([
